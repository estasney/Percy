...
def related():
    if request.method == 'GET':
        if not request.args:
            return render_template('percy/related.html')
        else:
            ...
            if result_success:
                return render_template('percy/related.html', result=result, success='True',
                                       title_h2='{} Similarity Score'.format(query_scope.title()),
                                       title_th='Similarity Score', original=user_query, scope=query_scope)
            else:
                return render_template('percy/related.html', result=user_query, success='False')
...
def keywords():
    return render_template('percy/keywords.html')


@bp.route('/thisplusthat', methods=['GET', 'POST'])
def thisplusthat():
    if request.method == 'GET':
        return render_template('percy/thisplusthat.html')
    elif request.method == 'POST':
        solution_success, solution = sims_tools.word_math(request)
        scope = request.form.get('scope')
        if solution_success:
            return render_template('percy/thisplusthat.html', result=solution['scores'], success='True',
                                   user_equation=solution['equation'], pos_words=solution['positives'],
                                   neg_words=solution['negatives'], unknown_words=solution['unknowns'],
                                   scope=scope)
        else:
            return render_template('percy/thisplusthat.html', success='False')


@bp.route('/tf_idf', methods=['GET', 'POST'])
def tfidf():
    if request.method == 'GET':
        return render_template('percy/tf_idf.html')
    elif request.method == 'POST':
        user_input = request.form['tfidf_text']
        fp = fingerprint_tools.Fingerprint()
        scored_tfidf = fp.fingerprint(user_input)
        return render_template('percy/tf_idf.html', success='True', original=user_input, result=scored_tfidf)
