...
@bp.route('/')
def open_page():
    return render_template('percy/home_page.html')
...
def lookup_name():
    if request.method == "GET":
        return render_template("percy/diversity_lookup.html")

    name_lookup = request.form.get('q', '')
    result = diversity_tools.get_name_count(name_lookup)
    if not result:
        return render_template('percy/diversity_lookup.html', success='False', original=name_lookup)
    return render_template('percy/diversity_lookup.html', success='True', original=name_lookup, result=result)


@bp.route('/diversity_analysis', methods=['GET', 'POST'])
def infer_diversity():
    if request.method == "GET":
        return render_template('percy/diversity_analysis.html')

    if request.files.get('file', None):
        file_upload = upload_tools.UploadManager(request)
        if not file_upload.status:
            return render_template('percy/diversity_analysis.html', success='False')
        names_list = file_upload.file_data()
        if not names_list:
            return render_template('percy/diversity_analysis.html', success='False', error_message=file_upload.status)
    else:
        names_list = request.form.get('paste_names', '')
        if not names_list:
            return render_template('percy/diversity_analysis.html', success='False',
                                   error_message="Unable to analyze names")
        else:
    ...
    if not result:
        return render_template("percy/diversity_analysis.html", success='False', error_message='No names recognized')

    return render_template('percy/diversity_analysis.html', result=result)
